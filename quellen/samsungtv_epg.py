"""Optionale, echte Programmdaten von Samsung TV Plus Deutschland -
AUTOMATISCH als vierter Fallback in der DE-Kaskade (nach Pluto TV,
tvmovie.de und hoerzu.de), kein eigenes sender.txt-Praefix noetig.

Datenquelle ist die frei zugaengliche, loginfreie XMLTV-Datei des
"SGKodi / Kodi Unlimited Support"-Grabbers
(https://kodi-unlimited-support.de/iptv/epg/samsungtv_de_guide.xml.gz) -
EINE komplette Datei mit allen deutschen Samsung-TV-Plus-Kanaelen UND
allen Sendungen darin (Titel, Sub-Title, Beschreibung), wird nur EINMAL
...
Deckt nur ca. 1 Tag im Voraus ab, Tage danach sind ohnehin immer
generisch.
...
URL = "https://kodi-unlimited-support.de/iptv/epg/samsungtv_de_guide.xml.gz"
...
def _xml_laden():
    """Laedt und parst (und cached) die komplette SamsungTV-Plus-DE-XMLTV-
    ...
    try:
        ...
        print(f"SamsungTV-EPG: {len(kanaele)} Kanaele, {len(programme)} Kanaele mit Sendungen geladen.")

        daten = {"kanaele": kanaele, "programme": programme}
        _daten_cache = daten
        return daten
    except Exception as e:
        print(f"SamsungTV-EPG: Laden/Parsen fehlgeschlagen ({e}), ueberspringe.")
...
def samsungtv_kanal_finden(kanalname):
    """Sucht den Samsung-TV-Plus-Kanal, der am besten zu kanalname passt -
...
def samsungtv_hole_programme(site_id, tage=1):
