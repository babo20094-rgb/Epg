...
try:
    ...
    if flo_response.status_code == 200:
        ...
        # Events ohne Live-Link aussortieren, dann Zeit-Labels in echte
        # Start-/Endzeiten umwandeln.
        #
        # Hinweis: floracing.com laedt die genauen Zeit-Badges pro
        # Event ("10:00 PM, Jul 25" etc.) clientseitig per Angular/JS
        # nach - sie stehen NICHT im vom Server ausgelieferten HTML
        # (bestaetigt durch Debug-Auswertung: keines der 15 gefundenen
        # Events hatte ein per Regex erkennbares Zeit-Label, obwohl
        # alle einen gueltigen Live-Link hatten). Titel + Live-Link
        # sind dagegen serverseitig vorhanden. Für Events ohne
        # erkennbares Zeit-Label, aber mit Live-Link, wird deshalb
        # ersatzweise "Live Now" angenommen (aktuelle Zeit + 4 Std.),
        # da das ohnehin die einzige verfuegbare Information ist.
        geparste_events = []
        for event in flo_events:
            if not event["hat_live_link"]:
                continue

            zeiten = flo_zeit_parsen(event["zeit_label"] or "Live Now, Jan 1", flo_jetzt)
            if not zeiten:
                continue
