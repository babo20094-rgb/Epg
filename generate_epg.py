        ...
        for daten in sender_daten:
            ...
            if verwende_event and tag_index == 0:
                ...
                if geparste_zeit:
                    ...
                    kandidat_ende = kandidat_start + timedelta(hours=2)
            ...
            episode_tag = ""
            if kategorie_key in EPISODEN_KATEGORIEN and not verwende_event:
            ...
            # Poster/Still-Bild passend zur Kategorie (zusaetzlich zum
            # Sender-Logo im <channel>-Tag). Faellt auf ein generisches
            # Unterhaltungs-Bild zurueck, wenn keine Kategorie erkannt
            # wurde.
            poster_tag = (
                f' <icon src="{escape(poster_fuer_kategorie(kategorie_key))}"/>'
            )
            ...
            xml_teile.append(
                ...
                f'{sub_title_tag}'
                f'{desc_tag}{category_tag}{rating_tag}{episode_tag}{poster_tag} </programme> '
            )
    ...
    if response.status_code == 200:
        ...
        if len(daten) == 0:
            ...
            for event in daten:
                ...
                # DYN-Live-Events sind durchweg Sport-Uebertragungen -
                # entsprechendes Poster fest zuordnen.
                poster_tag = f' <icon src="{escape(poster_fuer_kategorie("SPORT"))}"/>'

                xml_teile.append(
                    f' <programme start="{startzeit}" stop="{endzeit}" channel="{escape(kanal)}">'
                    f' <title>{escape(titel)}</title> <desc>{escape(beschreibung)}</desc>{poster_tag} </programme> '
                )
