...
try:
    ...
    if flo_response.status_code == 200:
        ...
        for titel_a in flo_soup.find_all("a", href=re.compile(r"/events/\d+-")):
            try:
                block = titel_a.find_parent("li") or titel_a.parent
                if block is None:
                    continue

                titel = titel_a.get_text(strip=True)
                if not titel:
                    continue

                live_a = block.find("a", href=re.compile(r"/live/\d+$"))
                if not live_a:
                    continue

                block_text = block.get_text(" ", strip=True)
                zeit_match = FLO_ZEIT_MUSTER.search(block_text)
                if not zeit_match:
                    continue

                zeiten = flo_zeit_parsen(zeit_match.group(0), flo_jetzt)
                if not zeiten:
                    continue

                start_utc, ende_utc = zeiten

                # Bereits vergangene Events (mit unserer groben
                # Zeitschätzung) nicht mehr anzeigen.
                if ende_utc < flo_jetzt:
                    continue

                kategorie_a = block.find("a", href=re.compile(r"^/collections/tag/"))
                kategorie = kategorie_a.get_text(strip=True) if kategorie_a else None
                ...
                # Ein einzelner kaputter Eintrag soll nicht die
                # komplette Auswertung abbrechen.
                continue
        ...
        print(f"Flo Racing: HTTP-Status {flo_response.status_code}, Leerzeiten werden verwendet")
