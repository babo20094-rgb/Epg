...
try:
    ...
    if flo_response.status_code == 200:
        ...
        print(
            f"Flo Racing Debug: Antwortlaenge {len(flo_response.text)} Zeichen."
        )
        ...
        for element in flo_soup.descendants:
            if isinstance(element, bs4.NavigableString):
                ...
                if text:
                    ...
                    elif re.search(r"live|now|[AP]M|\d{1,2}:\d{2}", text, re.IGNORECASE) and len(_dbg_rohe_zeit_texte) < 25:
                        _dbg_rohe_zeit_texte.append(text)
                continue
        ...
        # --- DEBUG ---
        # Zeigt, wie viele Titel-Links (/events/...) ueberhaupt
        # gefunden wurden und wie viele davon Zeit-Label bzw.
        # Live-Link haben - hilft zu erkennen, ob sich das HTML
        # geaendert hat oder ob einfach gerade keine Events laufen.
        print(f"Flo Racing Debug: {len(flo_events)} roher Titel-Kandidat(en) gefunden.")
        for _dbg_event in flo_events[:5]:
            print(
                f"  - Titel={_dbg_event['titel']!r} "
                f"Zeit-Label={_dbg_event['zeit_label']!r} "
                f"Live-Link={_dbg_event['hat_live_link']}"
            )
        print(f"Flo Racing Debug: rohe zeitaehnliche Textschnipsel (max 25): {_dbg_rohe_zeit_texte}")
