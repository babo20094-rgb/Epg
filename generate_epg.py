...
for zeile in logo_only_zeilen:
    ...
    ist_exakt_praefix = zeile.upper().startswith("EXAKT:")
    roh_teile = zeile.split("|")

    if ist_exakt_praefix:
        # EXAKT: Name muss GENAU (Zeichen für Zeichen) mit dem
        # aktuellen Playlist-Namen übereinstimmen, sonst matcht nichts
        # mehr (z.B. wenn sich der Name durch ein Live-Event ändert).
        # Überspringt bewusst die Teilstring-Suche gegen sender.txt UND
        # die DYN-PPV-Erkennung, damit z.B. Namen wie
        # "- NO EVENT STREAMING - | 8K EXCLUSIVE | DE: DYN PPV 1" nicht
        # fälschlich als Logo-Override für die bestehenden
        # DYN PPV 1-20 HD Live-Event-Kanäle interpretiert werden.
        rest = zeile[6:]
        ...
        if kanal not in gesehene_logo_only_kanaele:
            ...
            print(
                f"logo_only.txt: '{voller_name}' als EXAKT-Kanal "
                f"(nur Icon, kein EPG) angelegt."
            )
