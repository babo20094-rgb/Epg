...
try:
    ...
    if flo_response.status_code == 200:
        ...
        # --- NUR ZU TESTZWECKEN ---
        # Testet, ob der .json-Endpunkt der Shopify-Collection-Seite
        # zugaenglich ist (oft weniger streng durch Bot-Schutz gefiltert
        # als die normale HTML-Seite). Es wird noch NICHTS geparst oder
        # verwendet, nur der Status + Antwort-Anfang ausgegeben, damit
        # wir sehen, ob dieser Weg funktioniert.
        try:
            _flo_json_test = _flo_session.get(
                "https://www.floracing.com/collections/6752029-live/event.json"
                "?view=live-and-upcoming",
                timeout=30,
            )
            print(
                f"Flo Racing JSON-Test: HTTP-Status "
                f"{_flo_json_test.status_code}, Antwort-Anfang: "
                f"{_flo_json_test.text[:300]!r}"
            )
        except Exception as _json_test_error:
            print("Flo Racing JSON-Test Fehler:", _json_test_error)
